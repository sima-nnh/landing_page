...
export const Footer = ({
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    log('info', 'Footer form submission started');
    ...
    try {
      ...
      setLoading(false);
      log('info', 'Footer form submission completed');
    }
  };
      ...
      {showSuccessModal && (
        <div className="footer-success-overlay" onClick={closeModal}>
          ...
          >
            ...
            <h3 className="footer-success-title">
              {isExistingUser ? "You're already subscribed!" : "Thanks for subscribing!"}
            </h3>
            <p className="footer-success-message">
              {isExistingUser ? (
                <>You've already signed up{submittedEmail && `, ${submittedEmail}`}! You'll receive updates shortly.</>
              ) : (
                <>You'll receive product updates, newsletters, and launch notes. No spam.</>
              )}
            </p>
            <button
